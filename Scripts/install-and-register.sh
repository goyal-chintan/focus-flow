...
# If app is already installed and checksums match, skip update
if [ -n "$INSTALLED_BINARY_CHECKSUM" ] && [ "$NEW_BINARY_CHECKSUM" = "$INSTALLED_BINARY_CHECKSUM" ]; then
...
if [ -n "$INSTALLED_BINARY_CHECKSUM" ]; then
  log_warn "App update available - will rebuild and reinstall"
fi
...
# Ad-hoc sign with stable bundle identifier so notification permissions attach
# to com.focusflow.app and show correctly in System Settings.
log_info "Code signing app bundle..."
dry_run_exec codesign --force --sign - --identifier com.focusflow.app --timestamp=none "$APP_BUNDLE"
...
if [ -e "$INSTALLED_APP" ]; then
  log_warn "Removing existing installation at $INSTALLED_APP"
